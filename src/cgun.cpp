...
CGun::CGun(ZTime *ztime_, ZSettings *zsettings_, bool just_placed) :
		ZCannon(ztime_, zsettings_) {
	hover_name = "Gun";
	object_name = "gun";

	InitTypeId(m_object_type, GUN);
...
void CGun::Init() {
	...
	for (i = 1; i < MAX_TEAM_TYPES; i++)
		for (j = 0; j < MAX_ANGLE_TYPES; j++) {
			sprintf(filename_c, "assets/units/cannons/gun/equiped_%s_r%03d.png", team_type_string[i].c_str(),
					ROTATION[j]);
...
void CGun::FireMissile(int x_, int y_) {
	...
	if (effect_list)
		effect_list->push_back(
				(ZEffect*) (new ELightRocket(ztime, x + 17 + sx[direction], y + 14 + sy[direction], x_, y_,
						missile_speed, 0, 1)));
...
void CGun::FireTurrentMissile(int x_, int y_, double offset_time) {
	if (effect_list)
		effect_list->insert(effect_list->begin(),
				(ZEffect*) (new ECannonDeath(ztime, loc.x, loc.y, ECANNONDEATH_GUN, x_, y_, offset_time)));
}
