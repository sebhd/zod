...
ZVehicle::ZVehicle(ZTime *ztime_, ZSettings *zsettings_) : ZMannedObject(ztime_, zsettings_) {
	object_name = "vehicle";
	m_object_type = VEHICLE_OBJECT;
	selectable = true;
	moving = false;
	next_move_time = 0;
	lid_open = false;
...
void ZVehicle::DoDriverHitEffect() {
...
void ZVehicle::Init() {
	...
	for (i = 1; i < MAX_TEAM_TYPES; i++) {
		for (j = 0; j < MAX_ANGLE_TYPES; j++) {
			for (k = 0; k < 2; k++) {
				sprintf(filename_c, "assets/units/robots/tank_fire_%s_r%03d_n%02d.png", team_type_string[i].c_str(),
						ROTATION[j], k);
...
void ZVehicle::TryDropTracks() {
	...
	if (the_time >= next_drop_track_time) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (zmap->CoordIsRoad(x[i], y[i]))
				continue;
			if (zmap->CoordIsRoad(x[i], y[i] - 16))
				continue; //up
			if (zmap->CoordIsRoad(x[i], y[i] + 16))
				continue; //down
			if (zmap->CoordIsRoad(x[i] - 16, y[i]))
				continue; //left
			if (zmap->CoordIsRoad(x[i] + 16, y[i]))
				continue; //right
		...
		if (lay[0] || lay[1]) {
			effect_list->insert(
					effect_list->begin(),
					(ZEffect*) (new ETrack(ztime, center_x, center_y, direction, track_type,
							zmap->GetMapBasics().terrain_type, lay[0], lay[1])));

			for (int i = 0; i < 2; i++)
				if (lay[i] && rand() % 4)
					effect_list->insert(
							effect_list->begin(),
							(ZEffect*) (new ETankDirt(ztime, x[i], y[i], zmap->GetMapBasics().terrain_type,
									direction == 0 || direction >= 4)));
		}

		//partially damaged smoke
		if (ShowPartiallyDamaged() && !(rand() % 3))
			effect_list->insert(effect_list->begin(),
					(ZEffect*) (new ETankSmoke(ztime, center_x, center_y, direction, false)));

		//fully damaged fire smoke, sparks and oil
		if (ShowDamaged()) {
			if (!(rand() % 3))
				effect_list->insert(effect_list->begin(),
						(ZEffect*) (new ETankSmoke(ztime, center_x, center_y, direction, true)));
			if (!(rand() % 16))
				effect_list->insert(effect_list->begin(),
						(ZEffect*) (new ETankOil(ztime, center_x, center_y, direction)));
			if (!(rand() % 48))
				effect_list->insert(effect_list->begin(),
						(ZEffect*) (new ETankSpark(ztime, center_x, center_y, direction)));
		}
...
void ZVehicle::SetAttackObject(ZObject *obj) {
	...
	if (attacked_object) {
		...
		attacked_object->GetCenterCords(tx, ty);
...
bool ZVehicle::CanBeRepaired() {
	if (IsDestroyed() || health >= max_health) {
		return false;
	}
...
void ZVehicle::SignalLidShouldClose() {
	...
	float lid_open_time_min = 2.0 / (GetSmartness());
	float lid_open_time_delay = 5.0 / (GetSmartness());
