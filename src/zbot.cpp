...
void ZBot::CollectOurTargets_3(vector<ZObject*> &targets_list, bool all_out) {
	for (vector<ZObject*>::iterator t = object_list.begin(); t != object_list.end(); t++) {
		...
		case VEHICLE_OBJECT:
			...
			manned = dynamic_cast<ZMannedObject*>(*t);

			if (manned && manned->CanBeEntered()) {
				targets_list.push_back(*t);
			}
			//attack object
...
void ZBot::GiveOutOrders_2(vector<ZObject*> unit_list, vector<ZObject*> target_list,
	...
	while (unit_list.size()) {
		for (vector<ZObject*>::iterator u = unit_list.begin(); u != unit_list.end();) {
			...
			//reset target list?
			if (!target_list.size())
				target_list = orig_target_list;
			if (!grenade_box_list.size())
				grenade_box_list = orig_grenade_box_list;
			...
			//empty unit?
			else if (closest_obj->GetOwner() == NULL_TEAM && (ot == CANNON_OBJECT || ot == VEHICLE_OBJECT)
			...
			//destroyed building?
					...
					&& ot == BUILDING_OBJECT && closest_obj->IsDestroyed()) {
	...
	while (units_list.size()) {
		...
		for (vector<ZObject*>::iterator u = units_list.begin(); u != units_list.end();) {
			...
			//find that targets nearest unit
			{
				...
				//didn't find one?
				if (!tc_obj)
					printf("ZBot::GiveOutOrders_3: !tc_obj\n");
				//do we match?
				else if (tc_obj != *u) {
					u++;
					continue;
			...
			//give command
			{
				...
				case BUILDING_OBJECT:
					...
					case FORT_BACK:
						//enter fort
						if (uc_obj->GetOwner() != our_team && uc_obj->GetOwner() != NULL_TEAM
								&& !uc_obj->IsDestroyed()) {
							new_waypoint.mode = ENTER_FORT_WP;
							set_waypoint = true;
						}
...
bool ZBot::Stage1AI() {
	...
	for (vector<ZObject*>::iterator o = object_list.begin(); o != object_list.end(); o++) {
		...
		if ((*o)->GetOwner() != NULL_TEAM && (*o)->GetOwner() != our_team)
			continue;
	...
	for (vector<ZObject*>::iterator o = object_list.begin(); o != object_list.end(); o++) {
		...
		//is it mobile
		if (!(oot == ROBOT_OBJECT || oot == VEHICLE_OBJECT))
			continue;
		...
		if ((*o)->GetWayPointList().size()) {
			...
			if ((*o)->GetWayPointList().begin()->mode == MOVE_WP) {
				...
				if (!(tot == MAP_ITEM_OBJECT && toid == FLAG_ITEM))
					continue;
				if (temp_obj->GetOwner() != our_team)
					continue;
			} else
				continue;
		}
		//if robot, is it a leader?
		if ((*o)->IsGroupMember() && (*o)->IsMinion())
			continue;
...
void ZBot::connect_event(char *data, int size, int dummy) {
	ProcessConnect();
	SendBotBypassData();
}
...
void ZBot::reset_game_event(char *data, int size, int dummy) {
	ProcessResetGame();
...
void ZBot::set_grenade_amount_event(char *data, int size, int dummy) {
	ProcessSetGrenadeState(data, size);
}


void ZBot::set_object_attack_object_event(char *data, int size, int dummy) {
	ProcessObjectAttackObject(data, size);
}


void ZBot::set_object_health_event(char *data, int size, int dummy) {
	ProcessObjectHealthTeam(data, size);
}

void ZBot::set_object_team_event(char *data, int size, int dummy)
{
	ProcessObjectTeam(data, size);
}


void ZBot::set_player_voteinfo_event(char *data, int size, int dummy) {
...
void ZBot::set_vote_info_event(char *data, int size, int dummy) {
	ProcessVoteInfo(data, size);
}

void ZBot::store_map_event(char *data, int size, int dummy) {
	ProcessMapDownload(data, size);
}

void ZBot::test_event(char *data, int size, int dummy) {
	if (size)
