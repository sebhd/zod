...
public:
	GWPFullUnitSelectorButton() {
		ot = oid = 0;
		object_button.SetType(OBJECT_BUTTON);
	}
...
public:
	...
	void SetBuildingType(int building_type_) {
	...
	void SetUnitSelectorRefID(int us_ref_id_) {
		us_ref_id = us_ref_id_;
	}
	...
	bool GetSelected(unsigned char &ot, unsigned char &oid) {
		ot = sot;
		oid = soid;
		return object_selected;
	}

	int GetUnitSelectorRefID() {
		return us_ref_id;
	}
	bool IsActive() {
		return is_active;
...
public:
	...
	void GetCoords(int &x_, int &y_) {
		x_ = x;
		y_ = y;
	}
	void SetActive(bool is_active_) {
	...
	void SetIsOnlySelector(bool value) {
		is_only_selector = value;
	}

	bool GetSelectedID(unsigned char &ot, unsigned char &oid);

	bool LoadFullSelector() {
		return load_fus;
	}
...
private:
	...
	bool IsWithinPortrait(int x, int y);
	...
	// The currently selected production object:
	// TODO 3: The name 'draw_obj' is confusing, since it is not only responsible for drawing,
	// but directly defines the production selection
	ZObject *draw_obj;
	...
	// Index of the object that is currently selected for production:
	int select_i;
...
public:
	...
	void SetIsExpanded(bool is_expanded_) {
