...
void ZPlayer::InitAnimals() {
	...
	for (int i = 0; i < birds; i++) {
		bird_list.push_back(
				new ABird(&ztime, &zsettings, (planet_type) zmap.GetMapBasics().terrain_type,
						zmap.GetMapBasics().width * 16, zmap.GetMapBasics().height * 16));
	}
}
...
void ZPlayer::InitSDL() {
	...
	if (use_opengl) {
		...
		if (is_windowed)
			...
			screen = SDL_SetVideoMode(init_w, init_h, 32,
	...
	if (!ttf_font)
		printf("could not load assets/arial.ttf\n");
...
void ZPlayer::SetPlayerTeam(team_type player_team) {
	...
	if (gui_factory_list)
		gui_factory_list->SetTeam(our_team);
...
void ZPlayer::ProcessGame() {
	...
	for (vector<ZEffect*>::iterator i = effect_list.begin(); i != effect_list.end(); i++) {
		...
		if ((*i)->GetEFlags().unit_particles)
			MissileObjectParticles((*i)->GetEFlags().x, (*i)->GetEFlags().y, (*i)->GetEFlags().unit_particles_radius,
					(*i)->GetEFlags().unit_particles_amount);
	}
...
void ZPlayer::ProcessVerbalWarnings() {
	...
	if (the_time >= next_your_losing_msg_time) {
		...
		if (next_worst_unit_count > our_unit_count && next_worst_territory_percentage > our_territory_percentage) {
			...
			if (select_info.can_repair && hover_object->CanBeRepairedByCrane(our_team)) {
				...
				if (select_info.can_move) {
					if (ot == MAP_ITEM_OBJECT && oid == GRENADES_ITEM && select_info.can_pickup_grenades)
						...
						cursor.SetCursor(GRAB_C);
					else if ((ot == CANNON_OBJECT || ot == VEHICLE_OBJECT) && hover_object->GetOwner() == NULL_TEAM
							&& select_info.can_equip)
						cursor.SetCursor(ENTER_C);
					else {
						if (hover_object_can_enter_fort)
							cursor.SetCursor(PLACE_C);
						else {
							if (!select_info.can_attack
									|| (!select_info.have_explosives && hover_object->AttackedOnlyByExplosives()))
				...
				ZMannedObject* manned = dynamic_cast<ZMannedObject*>(hover_object);

				if (manned) {

					if (manned->GetEjectable() && select_info.selected_list.size() == 1 && i != -1) {
...
void ZPlayer::ProcessSDL() {
	...
	while (SDL_PollEvent(&event))
		...
		case SDL_VIDEORESIZE:
			...
			eventHandler.ProcessEvent(SDL_EVENT, RESIZE_EVENT, NULL, 0, 0);
			break;
		case SDL_MOUSEMOTION:
			...
			eventHandler.ProcessEvent(SDL_EVENT, MOTION_EVENT, NULL, 0, 0);
			break;
		case SDL_MOUSEBUTTONDOWN:
			...
			case SDL_BUTTON_LEFT:
				...
				eventHandler.ProcessEvent(SDL_EVENT, LCLICK_EVENT, NULL, 0, 0);
				break;
			case SDL_BUTTON_RIGHT:
				...
				eventHandler.ProcessEvent(SDL_EVENT, RCLICK_EVENT, NULL, 0, 0);
				break;
			case SDL_BUTTON_MIDDLE:
				...
				eventHandler.ProcessEvent(SDL_EVENT, MCLICK_EVENT, NULL, 0, 0);
				break;
			case SDL_BUTTON_WHEELUP:
				//ehandler.AddEvent(new Event(SDL_EVENT, WHEELUP_EVENT, 0, NULL, 0));
				eventHandler.ProcessEvent(SDL_EVENT, WHEELUP_EVENT, NULL, 0, 0);
				break;
			case SDL_BUTTON_WHEELDOWN:
				//ehandler.AddEvent(new Event(SDL_EVENT, WHEELDOWN_EVENT, 0, NULL, 0));
				eventHandler.ProcessEvent(SDL_EVENT, WHEELDOWN_EVENT, NULL, 0, 0);
				break;
			}
			break;
		case SDL_MOUSEBUTTONUP:
			...
			case SDL_BUTTON_LEFT:
				...
				eventHandler.ProcessEvent(SDL_EVENT, LUNCLICK_EVENT, NULL, 0, 0);
				break;
			case SDL_BUTTON_RIGHT:
				//ehandler.AddEvent(new Event(SDL_EVENT, RUNCLICK_EVENT, 0, NULL, 0));
				eventHandler.ProcessEvent(SDL_EVENT, RUNCLICK_EVENT, NULL, 0, 0);
				break;
			case SDL_BUTTON_MIDDLE:
				//ehandler.AddEvent(new Event(SDL_EVENT, MUNCLICK_EVENT, 0, NULL, 0));
				eventHandler.ProcessEvent(SDL_EVENT, MUNCLICK_EVENT, NULL, 0, 0);
				break;
			}
			break;
		case SDL_KEYDOWN:
			...
			eventHandler.ProcessEvent(SDL_EVENT, KEYDOWN_EVENT_, (char*) &the_key, sizeof(key_event), 0);
		...
		case SDL_KEYUP:
			...
			eventHandler.ProcessEvent(SDL_EVENT, KEYUP_EVENT_, (char*) &the_key, sizeof(key_event), 0);
...
void ZPlayer::SelectAllOfType(int type) {
	...
	for (vector<ZObject*>::iterator i = ols.passive_engagable_olist.begin(); i != ols.passive_engagable_olist.end();
			++i) {
		unsigned char ot, oid;
...
bool ZPlayer::UnitNearHostiles(ZObject *obj) {
	...
	for (vector<ZObject*>::iterator i = ols.passive_engagable_olist.begin(); i != ols.passive_engagable_olist.end();
		...
		if (obj == eobj)
			continue;
		if (obj->GetOwner() == eobj->GetOwner())
			continue;
		if (eobj->GetOwner() == NULL_TEAM)
			continue;
		if (eobj->IsDestroyed())
			continue;
		if (!obj->WithinAgroRadius(eobj))
			continue;
		...
		if (hover_object && !coords_from_mini_map) {
			...
			if ((*i)->CanBeRepaired() && hover_object->CanRepairUnit(our_team)) {
				...
				case ROBOT_OBJECT:
					...
					//capturable item?
					if (hover_object->GetOwner() == NULL_TEAM && (hot == CANNON_OBJECT || hot == VEHICLE_OBJECT)) {
...
bool ZPlayer::GuiAbsorbLClick() {
	...
	// If the mouse is hovering over an object, open it's gui window:
	// NOTE: This has been moved to GuiAbsorbLUnClick()
	/*
...
bool ZPlayer::GuiAbsorbLUnClick() {
	...
	if (active_menu) {
		...
		if (active_menu->UnClick(map_x, map_y)) {
			...
			if (glfags.do_createuser) {
				SendCreateUser(active_menu->GetGFlags().user_name, active_menu->GetGFlags().login_name,
						active_menu->GetGFlags().login_password, active_menu->GetGFlags().email);
			}
	...
	if (gui_window) {
		...
		if (gui_window->UnClick(map_x, map_y)) {
			...
			// If some user action in the gui window ordered an addition to the build queue...
			...
			// If some user action in the gui window ordered an addition to the build queue...
			if (gui_window->GetGFlags().send_replace_production) {
				start_building_packet the_data;

				the_data.ref_id = gui_window->GetGFlags().rref_id;
				the_data.ot = gui_window->GetGFlags().rot;
				the_data.oid = gui_window->GetGFlags().roid;

				client_socket.SendMessage(REPLACE_BUILDING, (const char*) &the_data, sizeof(start_building_packet));
			}

			// Send STOP_BUILDING message:
			...
			if (gui_window->GetGFlags().send_new_queue_item) {
				...
				client_socket.SendMessage(ADD_BUILDING_QUEUE, (const char*) &the_data,
						sizeof(add_building_queue_packet));
			}
			...
			if (gui_window->GetGFlags().send_cancel_queue_item) {
				...
				client_socket.SendMessage(CANCEL_BUILDING_QUEUE, (const char*) &the_data,
						sizeof(cancel_building_queue_packet));
			}
	...
	if (click_object == hover_object) {
		return ObjectMakeGuiWindow(click_object);
	}
...
void ZPlayer::LoadMainMenu(int menu_type, bool kill_if_open, gmm_warning_flag warning_flags) {
	...
	case GMM_WARNING:
		...
		break;
	default:
		printf("ZPlayer::LoadMainMenu: bad menu_type:%d\n", menu_type);
		break;
	}
...
void ZPlayer::SetSoundSetting(int sound_setting_) {
	...
	if (sound_setting < 0)
		sound_setting = 0;
	if (sound_setting >= MAX_SOUND_SETTINGS)
		sound_setting = 0;
...
void ZPlayer::add_new_object_event(char *data, int size, int dummy) {
	...
	if (obj) {
		...
		if (ot == BUILDING_OBJECT && (oid == FORT_FRONT || oid == FORT_BACK) && obj->GetOwner() == our_team) {
			fort_ref_id = obj->GetRefID();
			FocusCameraToFort();
		}
		...
		//set effect list
		obj->SetEffectList(&GetEffectList());
	}
...
void ZPlayer::destroy_object_event(char *data, int size, int dummy) {
	...
	for (int i = 0; i < pi->fire_missile_amount; i++) {
		...
		memcpy((char*) &missile_info, data + sizeof(destroy_object_packet) + (sizeof(fire_missile_info) * i),
				sizeof(fire_missile_info));
	...
	//is it the last enemy fort?
	{
		...
		for (int i = RED_TEAM; i < MAX_TEAM_TYPES; i++)
			if (i != our_team && i != obj->GetOwner() && team_units_available[i] > 0) {
		...
		if ((ot == BUILDING_OBJECT && (oid == FORT_FRONT || oid == FORT_BACK)) && obj->GetOwner() != our_team
				&& team_units_available[our_team] > 0 && !other_teams_exist) {
			int x, y;
			obj->GetCenterCords(x, y);
			FocusCameraTo(x, y);
		}
	...
	if (pi->killer_ref_id != -1 && zhud.GetARefID() != -1 && !zhud.GetAPortrait().DoingAnim()) {
		ZObject *aobj;
		ZObject *kobj;

		aobj = ZObject::GetObjectFromID(zhud.GetARefID(), object_list);
		kobj = ZObject::GetObjectFromID(pi->killer_ref_id, object_list);

		//is the killer one of us and near our "A" unit?
		if (aobj && kobj && (kobj->GetOwner() == our_team)) {
			int ax, ay, kx, ky;
			...
			if (aobj == kobj || points_within_distance(ax, ay, kx, ky, 100)) {
				...
				zhud.GetAPortrait().SetObject(kobj);
				zhud.GetAPortrait().StartAnim(GOOD_HIT_ANIM + next_anim);
			}
		}
	}
...
void ZPlayer::display_login_event(char *data, int size, int dummy) {
	loginoff_packet *pi = (loginoff_packet*) data;

	//good packet?
	if (size != sizeof(loginoff_packet))
		return;

	if (pi->show_login) {
		//already shown?
		if (active_menu == login_menu)
			return;
		if (active_menu == create_user_menu)
			return;
...
void ZPlayer::display_news_event(char *data, int size, int dummy) {
	...
	AddNewsEntry(new_entry.message, new_entry.r, new_entry.g, new_entry.b);
	...
	/*
	 ...
	 textcolor.g = new_entry->g;
	 textcolor.b = new_entry->b;
	 //new_entry.text_image = TTF_RenderText_Solid(ttf_font, new_entry.message.c_str(), textcolor);
	 //new_entry->text_image.LoadBaseImage(TTF_RenderText_Solid(ttf_font, new_entry->message.c_str(), textcolor));
	 new_entry->text_image.LoadBaseImage(ZFontEngine::GetFont(SMALL_WHITE_FONT).Render(new_entry->message.c_str()));
...
void ZPlayer::do_crane_anim_event(char *data, int size, int dummy) {
...
void ZPlayer::do_portrait_anim_event(char *data, int size, int dummy) {
	...
	if (obj->GetOwner() != our_team)
		return;
	if (zhud.GetAPortrait().DoingAnim())
		return;

	zhud.GetAPortrait().SetObject(obj);

	//do the anim
	switch (pi->anim_id) {
	case VEHICLE_CAPTURED_ANIM:
		zhud.GetAPortrait().StartAnim(pi->anim_id);
		break;
	case GUN_CAPTURED_ANIM:
		zhud.GetAPortrait().StartAnim(pi->anim_id);
		break;
	case TERRITORY_TAKEN_ANIM:
		zhud.GetAPortrait().StartAnim(pi->anim_id);
		break;
	case TARGET_DESTROYED_ANIM:
		zhud.GetAPortrait().StartAnim(pi->anim_id);
		break;
	}

	//store space bar event
	AddSpaceBarEvent(SpaceBarEvent(pi->ref_id, true));
}

void ZPlayer::driver_hit_effect_event(char *data, int size, int dummy) {
...
void ZPlayer::get_version_event(char *data, int size, int dummy) {
	version_packet *pi = (version_packet*) data;
	char temp_version[MAX_VERSION_PACKET_CHARS];
	string message;

	//good packet?
	if (size != sizeof(version_packet))
		return;
...
void ZPlayer::keydown_event(char *data, int size, int dummy) {
	key_event *pi = (key_event*) data;
	int the_key = pi->the_key;
	int the_unicode = pi->the_unicode;

	if (the_unicode)
		ProcessUnicode(the_unicode);

	//printf("keydown:%d unicode:%d\n", the_key, the_unicode);

	SetAsciiState(the_key, true);
	...
	case '9':
		if (CtrlDown())
			select_info.SetGroup(the_key - '0');
		else {
			LoadControlGroup(the_key - '0');
			//select_info.LoadGroup(the_key - '0');
			//DetermineCursor();
			//ClearDevWayPointsOfSelected();
			//GiveHudSelected();
		}
...
void ZPlayer::keyup_event(char *data, int size, int dummy) {
	key_event *pi = (key_event*) data;
	int the_key = pi->the_key;
	int the_unicode = pi->the_unicode;
	//printf("keyup:%d\n", the_key);

	SetAsciiState(the_key, false);
...
void ZPlayer::lclick_event(char *data, int size, int dummy) {
	hud_click_response hud_response;

	lbutton.down = true;

	//taken by a main menu?
	if (MainMenuAbsorbLClick()) {
		lbutton.started_over_gui = true;
		return;
	}

	//zhud.LeftClick(mouse_x, mouse_y, screen->w, screen->h, hud_response);
	zhud.LeftClick(mouse_x, mouse_y, init_w, init_h, hud_response);

	//did the hud absorb the click?
	if (!hud_response.used) {
		lbutton.started_over_hud = false;

		//see if gui absorbs it
		if (place_cannon) {

		} else if (GuiAbsorbLClick()) {
			lbutton.started_over_gui = true;

		} else if (zcomp_msg.AbsorbedLClick(mouse_x, mouse_y, zmap)) {

			lbutton.started_over_gui = true;
		} else {

			lbutton.started_over_gui = false;
			//normal click
			select_info.Clear();
			DetermineCursor();
			GiveHudSelected();
		...
		case HUD_BUTTON:
			break;
		case HUD_MINI_MAP:
			FocusCameraTo(hud_response.mini_x, hud_response.mini_y);
			break;
		case HUD_JUMP_TO_UNIT: {
			...
			if (!select_info.ObjectIsSelected(obj)) {
				SelectZObject(obj);
			}
		}
			break;
...
void ZPlayer::lunclick_event(char *data, int size, int dummy) {
	hud_click_response hud_response;

	//taken by a main menu?
	if (MainMenuAbsorbLUnClick()) {
		lbutton.down = false;
		return;
	}

	//zhud.LeftUnclick(mouse_x, mouse_y, screen->w, screen->h, hud_response);
	zhud.LeftUnclick(mouse_x, mouse_y, init_w, init_h, hud_response);
	...
	if (!hud_response.used) {
		...
		if (DoPlaceCannon()) {
			...
			if (zcomp_msg.GetFlags().resume_game) {
				...
				if (obj) {
					//select it?
					if (zcomp_msg.GetFlags().select_obj)
						SelectZObject(obj);
					//{
					//	//SelectZObject(obj);
					//	//DetermineCursor();
					...
					//open a gui?
					if (zcomp_msg.GetFlags().open_gui) {
						ObjectMakeGuiWindow(obj);
						//DeleteCurrentGuiWindow();
						//if(obj->GetOwner() != NULL_TEAM && obj->GetOwner() == our_team && (gui_window = obj->MakeGuiWindow()))
						//{
	...
	//collect selectables
	if (!lbutton.started_over_hud && !lbutton.started_over_gui) {
		CollectSelectables();
		lbutton.down = false;
		DetermineCursor();
		ClearDevWayPointsOfSelected();
	...
	switch (hud_response.type) {
	case HUD_BUTTON:
		HandleButton((hud_buttons) hud_response.button);
		break;
	case HUD_MINI_MAP:
		break;
	case HUD_JUMP_TO_UNIT:
...
void ZPlayer::mclick_event(char *data, int size, int dummy) {
	mbutton.down = true;
}

void ZPlayer::motion_event(char *data, int size, int dummy) {
	...
	if (mbutton.down) {
		...
		change_x = mouse_x - mbutton.x;
		change_y = mouse_y - mbutton.y;

		if (!change_x && !change_y)
			return;

		mbutton.map_x += change_x;
		mbutton.map_y += change_y;

		FocusCameraTo(mbutton.map_x, mbutton.map_y);

		//move the mouse back
		SDL_EventState(SDL_MOUSEMOTION, SDL_IGNORE);
		SDL_WarpMouse(mbutton.x, mbutton.y);
		SDL_EventState(SDL_MOUSEMOTION, SDL_ENABLE);
		mouse_x = mbutton.x;
		mouse_y = mbutton.y;
		return;
	...
	if (mouse_x < init_w - HUD_WIDTH && mouse_y < init_h - HUD_HEIGHT)
	{
		...
		for (i = object_list.begin(); i != object_list.end(); i++)
			if ((*i)->UnderCursor(map_x, map_y)) {
				hover_object_can_attack = (*i)->UnderCursorCanAttack(map_x, map_y);
				hover_object_can_enter_fort = (*i)->UnderCursorFortCanEnter(map_x, map_y);

				//if we can not attack or enter
				//then we aren't hovering over this object
				if (!hover_object_can_attack && !hover_object_can_enter_fort)
					continue;

				hover_object = (*i);
				//printf("object under cursor:%s\n", hover_object->GetObjectName().c_str());
			}

		if (!lbutton.started_over_hud && !lbutton.started_over_gui)
			CollectSelectables();
	} else {
		if (lbutton.down && lbutton.started_over_hud) {
			hud_click_response hud_response;

			zhud.MouseMotion(mouse_x, mouse_y, init_w, init_h, hud_response);
...
void ZPlayer::pickup_grenade_event(char *data, int size, int dummy) {
	...
	if (obj->GetOwner() == our_team && !zhud.GetAPortrait().DoingAnim()) {
		zhud.GetAPortrait().SetObject(obj);
		zhud.GetAPortrait().StartAnim(GRENADES_COLLECTED_ANIM);
...
void ZPlayer::poll_buy_regkey(char *data, int size, int dummy) {
	if (size)
		return;

	if (is_registered) {
		AddNewsEntry("the zod engine is already registered on this computer");
	} else {
		std::string s = "registration.zkey";

		if (!file_can_be_written(s.c_str())) {
			AddNewsEntry(
		...
		client_socket.SendMessage(BUY_REGKEY, (const char*) &packet, sizeof(buy_registration_packet));
	}
}

void ZPlayer::rclick_event(char *data, int size, int dummy) {
	rbutton.down = true;
}

void ZPlayer::request_version_event(char *data, int size, int dummy) {
	...
	client_socket.SendMessage(GIVE_VERSION, (const char*) &packet, sizeof(version_packet));
}

void ZPlayer::reset_game_event(char *data, int size, int dummy) {
	ProcessResetGame();
}

void ZPlayer::resize_event(char *data, int size, int dummy) {
	if (use_opengl) {
		if (is_windowed)
			SDL_SetVideoMode(init_w, init_h, 0, SDL_OPENGL | SDL_RESIZABLE);
		else
			SDL_SetVideoMode(init_w, init_h, 0, SDL_OPENGL | SDL_FULLSCREEN);

		ResetOpenGLViewPort(init_w, init_h);
	} else {
		if (is_windowed)
			SDL_SetVideoMode(init_w, init_h, 32, SDL_HWSURFACE | SDL_DOUBLEBUF | SDL_RESIZABLE);
		else
			SDL_SetVideoMode(init_w, init_h, 32, SDL_HWSURFACE | SDL_DOUBLEBUF | SDL_RESIZABLE | SDL_FULLSCREEN);
	}
	...
	//move menus around
	if (prev_w && prev_h)
		MainMenuMove(1.0 * init_w / prev_w, 1.0 * init_h / prev_h);
	prev_w = init_w;
	prev_h = init_h;
...
void ZPlayer::runclick_event(char *data, int size, int dummy) {
	...
	if (!ShiftDown()) {
		...
		if (select_info.selected_list.size() == 1) {
			...
			ZObject* i = select_info.selected_list[0];
			...
			// Eject driver if possible:
			ZMannedObject* manned = dynamic_cast<ZMannedObject*>((ZObject*) i);

			if (manned) {

				if (manned->GetEjectable() && manned->UnderCursor(mouse_x_map, mouse_y_map)) {
...
void ZPlayer::set_computer_message_event(char *data, int size, int dummy) {
	...
	//good packet?
	if (size != sizeof(computer_msg_packet))
		return;
	...
	case COMP_VEHICLE_SND:
		...
		zcomp_msg.DisplayMessage(VEHICLE_MANUFACTURED_MSG, pi->ref_id);
		AddSpaceBarEvent(SpaceBarEvent(pi->ref_id, true));
		break;
	case COMP_ROBOT_SND:
		ZSoundEngine::PlayWav(pi->sound);
		zcomp_msg.DisplayMessage(ROBOT_MANUFACTURED_MSG, pi->ref_id);
		AddSpaceBarEvent(SpaceBarEvent(pi->ref_id, true));
		break;
	case COMP_GUN_SND:
		ZSoundEngine::PlayWav(pi->sound);
		zcomp_msg.DisplayMessage(GUN_MANUFACTURED_MSG, pi->ref_id);
		AddSpaceBarEvent(SpaceBarEvent(pi->ref_id, false, true));
		break;
...
void ZPlayer::set_object_attack_object_event(char *data, int size, int dummy) {
	...
	if (obj->GetAttackObject() && obj->GetAttackObject()->GetOwner() == our_team && zhud.GetARefID() == -1
			&& !(rand() % 5)) {
		zhud.SetARefID(obj->GetAttackObject()->GetRefID());

		if (!zhud.GetAPortrait().DoingAnim()) {
			zhud.GetAPortrait().SetObject(obj->GetAttackObject());
			zhud.GetAPortrait().StartAnim(WERE_UNDER_ATTACK_ANIM);
		}

		//add this event to the spacebar event list
		AddSpaceBarEvent(SpaceBarEvent(obj->GetAttackObject()->GetRefID(), true));
	}
}
...
void ZPlayer::set_object_waypoints_event(char *data, int size, int dummy) {
	ZObject *our_object = ProcessWaypointData(data, size);
...
void ZPlayer::set_player_voteinfo_event(char *data, int size, int dummy) {
	...
	if (zvote.VoteInProgress()) {
		zvote.SetupImages(OurPInfo().real_voting_power(), VotesNeeded(), VotesFor(), VotesAgainst(),
				VoteAppendDescription());
	}
...
void ZPlayer::set_regkey(char *data, int size, int dummy) {
	...
	if (is_registered) {
		...
		if (!fp) {
			AddNewsEntry("new registration key bought but could not save to file! please visit www.nighsoft.com");
			return;
		}
...
void ZPlayer::snipe_object_event(char *data, int size, int dummy) {
	...
	effect_list.push_back((ZEffect*) (new ERobotTurrent(&(ztime), cx, cy - 4, obj->GetOwner())));
}

void ZPlayer::store_map_event(char *data, int size, int dummy) {
	ProcessMapDownload(data, size);

	if (zmap.Loaded()) {
		zmap.SetViewingDimensions(init_w - 100, init_h - 36);
		zhud.SetTerrainType((planet_type) zmap.GetMapBasics().terrain_type);
		zhud.GetMiniMap().Setup_Boundaries();
		InitAnimals();
...
void ZPlayer::team_ended_event(char *data, int size, int dummy) {
	team_ended_packet *pi = (team_ended_packet*) data;

	//good packet?
	if (size != sizeof(team_ended_packet))
		return;
	...
	for (unsigned int ii = 0; ii < object_list.size(); ++ii) {

		ZObject* obj = object_list[ii];
		...
		if (obj->GetOwner() == our_team) {
			...
			switch (ot) {
			case CANNON_OBJECT:
				end_animations.push_back(zhud_end_unit(ot, oid, manned->GetDriverType()));
				break;
			case VEHICLE_OBJECT:
				end_animations.push_back(zhud_end_unit(ot, oid, manned->GetDriverType()));
				break;
	...
	zhud.StartEndAnimations(end_animations, pi->won);
}

void ZPlayer::test_event(char *data, int size, int dummy) {
	if (size)
		printf("ZPlayer::test_event:%s...\n", data);
}

void ZPlayer::wheeldown_event(char *data, int size, int dummy) {
	MainMenuWheelDown();
	if (active_menu)
		active_menu->WheelDownButton();
	if (gui_window)
		gui_window->WheelDownButton();
	if (gui_factory_list)
		gui_factory_list->WheelDownButton();
}

void ZPlayer::wheelup_event(char *data, int size, int dummy) {
	MainMenuWheelUp();
	if (active_menu)
		active_menu->WheelUpButton();
	if (gui_window)
		gui_window->WheelUpButton();
	if (gui_factory_list)
		gui_factory_list->WheelUpButton();
}
