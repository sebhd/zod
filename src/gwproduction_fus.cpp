...
void GWPFullUnitSelector::DoRender(ZMap &the_map, SDL_Surface *dest, int shift_x, int shift_y) {
	...
	//render lists
	{
		...
		if (robot_list.size()) {
			DoRenderList(the_map, dest, lx, ly, robot_list);
			ly += GWPFUS_OBJH + GWPFUS_MARGIN;
		}
		if (vehicle_list.size()) {
			DoRenderList(the_map, dest, lx, ly, vehicle_list);
			ly += GWPFUS_OBJH + GWPFUS_MARGIN;
		}
		if (cannon_list.size()) {
			DoRenderList(the_map, dest, lx, ly, cannon_list);
			ly += GWPFUS_OBJH + GWPFUS_MARGIN;
		}
	}
}
...
void GWPFullUnitSelector::DoRenderList(ZMap &the_map, SDL_Surface *dest, int lx, int ly, vector<ZObject*> &the_list) {
	for (vector<ZObject*>::iterator o = the_list.begin(); o != the_list.end(); ++o) {
		...
		if (draw_obj->CreateHoverNameImages().GetBaseSurface()) {
			the_map.RenderZSurface(&draw_obj->CreateHoverNameImages(),
					lx + (28 - 5) - (draw_obj->CreateHoverNameImages().GetBaseSurface()->w >> 1), ly + (61 - 21));
		}
...
void GWPFullUnitSelector::LoadButtonList() {
	...
	int lx = GWPFUS_SIDE_SIZE + GWPFUS_MARGIN;
	int ly = GWPFUS_TOPH + GWPFUS_MARGIN;

	if (robot_list.size()) {
		AppendButtonList(lx, ly, robot_list);
		ly += GWPFUS_OBJH + GWPFUS_MARGIN;
	}
	if (vehicle_list.size()) {
		AppendButtonList(lx, ly, vehicle_list);
		ly += GWPFUS_OBJH + GWPFUS_MARGIN;
	}
	if (cannon_list.size()) {
		AppendButtonList(lx, ly, cannon_list);
		ly += GWPFUS_OBJH + GWPFUS_MARGIN;
	}
}
...
void GWPFullUnitSelector::AppendButtonList(int lx, int ly, vector<ZObject*> &the_list) {
	...
	for (vector<ZObject*>::iterator iter = the_list.begin(); iter != the_list.end(); ++iter) {

		GWPFullUnitSelectorButton new_button;

		// Fetch the button's object type and object id from object that the iterator points to:
		(*iter)->GetObjectID(new_button.ot, new_button.oid);
...
void GWPFullUnitSelector::CalculateWH() {
	...
	if (robot_list.size() > blocks_right)
		blocks_right = robot_list.size();
	if (vehicle_list.size() > blocks_right)
		blocks_right = vehicle_list.size();
	if (cannon_list.size() > blocks_right)
		blocks_right = cannon_list.size();
...
void GWPFullUnitSelector::LoadLists() {
	...
	//checks
	if (!buildlist || !building_obj) {
		return;
	}

	//get building level:
	int building_level = building_obj->GetLevel();

	//already loaded this type/level?
	if (building_type == lists_building_type && lists_building_level == building_level) {
		return;
	}
...
void GWPFullUnitSelector::LoadUnitToLists(unsigned char ot, unsigned char oid) {
	...
	case ROBOT_OBJECT:
		...
		break;
	case VEHICLE_OBJECT:
		the_list = &vehicle_list;
		break;
	case CANNON_OBJECT:
		the_list = &cannon_list;
		break;
	default:
		printf("GWPFullUnitSelector::LoadUnitToLists::tried to add invalid object %d %d\n", ot, oid);
		break;
	}
...
bool GWPFullUnitSelector::Click(int x_, int y_) {
	...
	// "Click" all unit selection buttons. Only the one under the mouse should react, though:
	for (vector<GWPFullUnitSelectorButton>::iterator i = button_list.begin(); i != button_list.end(); ++i) {
...
bool GWPFullUnitSelector::UnClick(int x_, int y_) {
	...
	// Unclick each button in the selector:
	for (vector<GWPFullUnitSelectorButton>::iterator i = button_list.begin(); i != button_list.end(); ++i) {

		if (i->object_button.UnClick(local_x, local_y)) {

			// If a button has been unclicked, set the respective object id/type:
