...
RSniper::RSniper(ZTime *ztime_, ZSettings *zsettings_) : ZRobot(ztime_, zsettings_) {
	...
	InitTypeId(m_object_type, SNIPER);
...
void RSniper::Init() {
	...
	for (i = 1; i < MAX_TEAM_TYPES; i++) {
		for (j = 0; j < MAX_ANGLE_TYPES; j++) {
			for (k = 0; k < 5; k++) {
				sprintf(filename_c, "assets/units/robots/grunt/fire_%s_r%03d_n%02d.png", team_type_string[i].c_str(),
						ROTATION[j], k);
...
void RSniper::DoRender(ZMap &the_map, SDL_Surface *dest, int shift_x, int shift_y) {
	...
	if (owner != NULL_TEAM) {
		switch (mode) {
		case R_WALKING:
			base_surface = &walk[owner][direction][move_i];
			break;
		case R_STANDING:
			base_surface = &stand[owner][direction];
			break;
		case R_BEER:
			base_surface = &beer[owner][action_i];
			break;
		case R_CIGARETTE:
			base_surface = &cigarette[owner][action_i];
			break;
		case R_FULLSCAN:
			base_surface = &full_area_scan[owner][action_i];
			break;
		case R_HEADSTRETCH:
			base_surface = &head_stretch[owner][action_i];
			break;
		case R_PICKUP_UP_GRENADES:
			base_surface = &pickup_up[owner][action_i];
			break;
		case R_PICKUP_DOWN_GRENADES:
			base_surface = &pickup_down[owner][action_i];
			break;
		case R_ATTACKING:
...
int RSniper::Process() {
	...
	if (mode == R_ATTACKING && the_time >= next_attack_time) {
		...
		if (action_i == 4 && m_attacked_object) {
			int tx, ty;
			int w, h;
			...
			if (effect_list)
				effect_list->push_back((ZEffect*) (new EBullet(ztime, owner, x + 8, y + 8, tx, ty)));
