...
ZSDL_Surface &ZObject::CreateHoverNameImages(bool force_update) {
	...
	if (hover_name.size()) {
		...
		// Draw the star behin the unit name in different color, depening on the unit's experience:
		...
		hover_name_img.LoadBaseImage(ZFontEngine::GetFont(font).Render(hover_name.c_str()));
		hover_name_star_img.LoadBaseImage(ZFontEngine::GetFont(font).Render((hover_name + " *").c_str()));
	}
...
int ZObject::DamageHealth(int damage_amount, ZMap &tmap) {
	...
	// TODO 3: Move this to overloaded virtual method
	RecalcBuildTime();
...
void ZObject::DoRenderWaypoints(ZMap &the_map, SDL_Surface *dest, vector<ZObject*> &object_list, bool is_rally_points,
...
unsigned char ZObject::GetExperience() {
	return experience;
}
...
ZSDL_Surface &ZObject::GetHoverNameImgStatic(unsigned char ot, unsigned char oid) {
	...
	if (ot < 0)
		return null_return;
	if (ot >= MAX_MAP_OBJECT_TYPES)
		return null_return;
	if (oid < 0)
		return null_return;
	if (oid >= max_units_in_type)
		return null_return;
...
int ZObject::GetSmartness() {
	ZUnit_Settings& unitSettings = zsettings->GetUnitSettings(m_object_type, m_object_id);
	return unitSettings.smartness * (experience + 1);
}

float ZObject::GetSnipeChance() {
	ZUnit_Settings& unitSettings = zsettings->GetUnitSettings(m_object_type, m_object_id);
...
void ZObject::InitTypeId(unsigned char ot, unsigned char oid) {
	...
	if (ot == BUILDING_OBJECT) {
		...
		ZUnit_Settings &unit_settings = zsettings->GetUnitSettings(m_object_type, m_object_id);
...
void ZObject::SetGroupExperience(unsigned char exp) {
	...
	if (leader) {
		for (vector<ZObject*>::iterator i = leader->GetMinionList().begin(); i != leader->GetMinionList().end(); i++) {
			(*i)->SetExperience(exp);
		}

		leader->SetExperience(exp);
	} else {
		for (vector<ZObject*>::iterator i = GetMinionList().begin(); i != GetMinionList().end(); i++) {
			(*i)->SetExperience(exp);
		}
...
void ZObject::RenderHover(ZMap &zmap, SDL_Surface *dest, team_type viewers_team) {
	 ...
	 * CreateHoverNameImages() will not produce images, so that the following attempt to use them will fail with segfault.
	 */
	if (!hover_name.size()) {
		return;
	}
	...
	if (!hover_name_img.GetBaseSurface() || !hover_name_star_img.GetBaseSurface()) {
		CreateHoverNameImages();
	}

	if (owner != NULL_TEAM) {
		int shiftX = (width_pix - hover_name_star_img.GetBaseSurface()->w) >> 1;
		zmap.RenderZSurface(&hover_name_star_img, loc.x + shiftX, loc.y + shiftY);
	} else {
		int shiftX = (width_pix - hover_name_img.GetBaseSurface()->w) >> 1;
		zmap.RenderZSurface(&hover_name_img, loc.x + shiftX, loc.y + shiftY);
	}
...
void ZObject::RenderHealth(ZMap &zmap, SDL_Surface *dest) {
	...
	if (yellow_dist - green_dist)
		if (zmap.GetBlitInfo(x + bar_x_shift + 1 + green_dist, y + bar_y_shift + 1, yellow_dist - green_dist, 2,
				from_rect, to_rect)) {
			the_box.x = to_rect.x;
			the_box.y = to_rect.y;
			the_box.w = from_rect.w;
...
void ZObject::GetObjectID(unsigned char &ref_object_type, unsigned char &ref_object_id) {
	ref_object_type = m_object_type;
	ref_object_id = m_object_id;
}
...
void ZObject::Engage(ZObject *target) {
	...
	//attack then
	if (m_attacked_object != target) {
		m_attacked_object = target;
		sflags.updated_attack_object = true;
...
void ZObject::CheckPassiveEngage(double &the_time, ZOLists &ols) {
	...
	if (owner == NULL_TEAM)
		return;
	if (!(m_object_type == CANNON_OBJECT || m_object_type == VEHICLE_OBJECT || m_object_type == ROBOT_OBJECT))
		return;
	if (m_object_type == ROBOT_OBJECT && IsMoving())
		return;
	...
	if (m_attacked_object) {
		...
		m_attacked_object->GetCenterCords(ox, oy);

		//if(!WithinAttackRadius(ox, oy))
		if (!WithinAttackRadius(m_attacked_object))
			Disengage();
	...
	if (CanAttack())
		for (obj = ols.passive_engagable_olist.begin(); obj != ols.passive_engagable_olist.end(); obj++) {
			...
			if ((*obj)->GetOwner() != NULL_TEAM && (*obj)->GetOwner() != owner && CanAttackObject((*obj))) {
				...
				if (!waypoint_list.size() && m_object_type != CANNON_OBJECT) {
					//if(WithinAgroRadius(ox, oy))
			...
			if (!waypoint_list.size() && m_object_type != CANNON_OBJECT) {
				if (!IsMinion() && !agro_choices.size()) {
					...
					if (manned) {
						if (m_object_type == ROBOT_OBJECT && manned->CanBeEntered() && WithinAutoEnterRadius(ox, oy)
								&& !(ot == VEHICLE_OBJECT && oid == APC)
						//&& !(this->just_left_cannon && ot == CANNON_OBJECT)) {
								&& !(this->just_disembarked)) {
...
bool ZObject::DodgeMissile(int tx, int ty, double time_till_explode) {
	...
	if (m_object_type == ROBOT_OBJECT && m_attacked_object) {
		return false;
	}
	...
	double dist = time_till_explode * real_move_speed;

	if (time_till_explode > stamina) {
		dist *= zsettings->run_unit_speed;
	}

	int m_move_dist = dist * 0.5;
	int f_move_dist = dist;
	...
	int nx = center_x + (move_dist * cos(theta));
	int ny = center_y + (move_dist * sin(theta));

	// Modify current dodge or make new?
	if (waypoint_list.size() && waypoint_list.begin()->mode == DODGE_WP) {

		// Altering this should make it recognize as a new waypoint later:
...
void ZObject::CreateAttackObjectData(char *&data, int &size) {
	...
	if (m_attacked_object)
		send_data.attack_object_ref_id = m_attacked_object->ref_id;
	else
...
void ZObject::ProcessAttackDamage(ZMap &tmap, bool attack_player_given) {
	...
	// Update time that has to pass until next damage:
	next_damage_time = the_time + damage_int_time;

	if (!this->CanAttackObject(m_attacked_object)) {
		Disengage();
		return;
	}
	...
	if (damage_is_missile || can_attack_with_grenades) {
		...
		if (!EstimateMissileTarget(m_attacked_object, tx, ty)) {
		...
		if (can_attack_with_grenades) {
			...
			new_missile.target_ref_id = m_attacked_object->GetRefID();
	...
	// If the attack is not a missile or a grenade...
	else {

		// Target missed?
		if ((rand() % 10000 / 10000.0 > GetDamageChance())) {
			return;
		}

		// Handle sniper attack:
		ZMannedObject* manned = dynamic_cast<ZMannedObject*>(m_attacked_object);

		// Did we snipe successfully?
		if (manned && can_snipe && manned->CanBeSniped() && ((rand() % 10000) / 10000.0 <= GetSnipeChance())) {
			manned->DamageDriverHealth(damage);
			sflags.updated_attack_object_driver_health = true;
		}

		// otherwise, do normal damage:
		else {
			m_attacked_object->DamageHealth(damage, tmap);
		...
		// TODO 4: Remove reference to pyro here? (replace with some virtual method call)
		if (m_object_type == ROBOT_OBJECT && m_object_id == PYRO) {
			m_attacked_object->SetDamagedByFireTime(the_time);
		}

		// TODO 4: Is it correct that the TARGET_DESTROYED_ANIM is only played when
		// the attack was not with missile/grenade?
		//if destroyed give a target destroyed
		if (m_attacked_object->IsDestroyed() && attack_player_given) {
...
void ZObject::ProcessKillObject() {
	...
	if (m_object_type == BUILDING_OBJECT && (m_object_id != FORT_FRONT && m_object_id != FORT_BACK)) {
		do_auto_repair = true;
...
void ZObject::ProcessAgroWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols, ZMap &tmap) {
	...
	if (!points_within_distance(ox, oy, cur_wp_info.agro_center_x, cur_wp_info.agro_center_y,
			attack_radius + zsettings->agro_distance)) {
		//go back to center
...
void ZObject::ProcessAttackWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols, ZMap &tmap) {
	...
	if (WithinAttackRadius(target_object)) {
		...
		if (cur_wp_info.path_finding_id)
			...
			//so start doing that
			if (!target_object->NearestAttackLoc(x, y, cur_wp_info.x, cur_wp_info.y, attack_radius,
					(m_object_type == ROBOT_OBJECT), tmap)) {
...
void ZObject::ProcessEnterWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols, ZMap &tmap) {
	...
	//target_object->GetOwner() != NULL_TEAM || target_object->IsDestroyed())

	ZMannedObject* manned = dynamic_cast<ZMannedObject*>(target_object);
...
void ZObject::ProcessMoveWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols, ZMap &tmap,
		bool stoppable) {
	const double z = 0.000001;
	int &x = loc.x;
	int &y = loc.y;
...
void ZObject::ProcessEnterFortWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols,
		ZMap &tmap) {
	int &x = loc.x;
	int &y = loc.y;
	float &dx = loc.dx;
...
void ZObject::ProcessUnitRepairWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols,
		ZMap &tmap) {
	const double z = 0.000001;
	int &x = loc.x;
	int &y = loc.y;
...
bool ZObject::CheckAttackTo(vector<waypoint>::iterator &wp, ZOLists &ols) {
	if (wp->attack_to) {
		...
		vector<ZObject*> agro_choices;
		for (vector<ZObject*>::iterator obj = ols.passive_engagable_olist.begin();
				obj != ols.passive_engagable_olist.end(); obj++) {
			int ox, oy;
			unsigned char ot, oid;
...
bool ZObject::ProcessMoveOrKillWP(double time_dif, ZMap &tmap, vector<waypoint>::iterator &wp, ZOLists &ols,
		bool stoppable) {
	int stop_x, stop_y;
...
bool ZObject::ProcessMove(double time_dif, ZMap &tmap, int &stop_x, int &stop_y, bool stoppable) {
	...
	if (stoppable
			&& tmap.WithinImpassable(inx + 1, iny + 1, width_pix - 2, height_pix - 2, stop_x, stop_y,
...
ZObject* ZObject::NextSelectableObjectAboveID(vector<ZObject*> &the_list, int unit_type, int only_team,
		int min_ref_id) {
	for (vector<ZObject*>::iterator i = the_list.begin(); i != the_list.end(); i++) {
		unsigned char ot, oid;
...
void ZObject::CloneMinionWayPoints() {
	for (vector<ZObject*>::iterator i = minion_list.begin(); i != minion_list.end(); i++) {
		...
		//just left cannon...
		(*i)->SetJustDisembarked(just_disembarked);
	}
}
...
void ZObject::ProcessGroupInfoData(char *data, int size, vector<ZObject*> &object_list) {
	...
	ZObject* leader_obj = GetObjectFromID(leader_ref_id, object_list);
	...
	for (int i = 0; i < minions; i++) {
		...
		if (new_minion) {
			leader_obj->AddGroupMinion(new_minion);
		} else {
...
bool ZObject::CanBeRepairedByCrane(int repairers_team) {
	if (m_object_type != BUILDING_OBJECT)
		return false;
	if (m_object_id == FORT_FRONT)
		return false;
	if (m_object_id == FORT_BACK)
		return false;
...
// TODO 3: Replace with member variable
