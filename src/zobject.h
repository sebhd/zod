...
enum object_mode {
	...
	R_BEER,
	R_ATTACKING,
	R_PICKUP_UP_GRENADES,
	R_PICKUP_DOWN_GRENADES,
	C_ATTACKING,
	MAX_OBJECT_MODES
};
...
public:
	...
	void clear_and_kill(ZMap &tmap) {
		if (path_finding_id)
			tmap.GetPathFinder().Kill_Thread_Id(path_finding_id);
		clear();
	}
};
...
public:
	...
	bool operator==(const waypoint &other) const {
		...
		if (other.mode != mode)
			return false;
		if (other.ref_id != ref_id)
			return false;
		if (other.x != x)
			return false;
		if (other.y != y)
			return false;
		if (other.attack_to != attack_to)
			return false;
		if (other.player_given != player_given)
			return false;
...
public:
	...
	static void SetDamageMissileList(vector<damage_missile> *damage_missile_list_) {
		damage_missile_list = damage_missile_list_;
	}

	string GetObjectName();
	void SetMap(ZMap *zmap_);
	void SetEffectList(vector<ZEffect*> *effect_list_) {
		effect_list = effect_list_;
	}
	virtual void SetOwner(team_type owner_);
	inline team_type GetOwner() {
		return owner;
	}
	inline void GetDimensions(int &w, int &h) {
		w = width;
		h = height;
	}
	inline void GetDimensionsPixel(int &w_pix, int &h_pix) {
		w_pix = width_pix;
		h_pix = height_pix;
	}
	virtual int Process();
...
// 		virtual SDL_Surface *GetRender();
	...
	static ZObject* NextSelectableObjectAboveID(vector<ZObject*> &the_list, int unit_type, int only_team,
			int min_ref_id);
...
//	void SetGameCore(ZCore* core) { gameCore = core; }
	...
	virtual bool CanSetRallypoints() {
		return false;
	}
	virtual bool CanMove();
	virtual bool CanPickupGrenades() {
		return false;
	}
	virtual bool CanHaveGrenades() {
		return false;
	}
	virtual void SetGrenadeAmount(int grenade_amount_) {
		return;
	}
	virtual int GetGrenadeAmount() {
		return 0;
	}
	...
	*/
	...
	inline void GetCords(int &x, int &y) {
		x = loc.x;
		y = loc.y;
	}
	inline void GetCenterCords(int &x, int &y) {
		x = center_x;
		y = center_y;
	}
	...
	vector<waypoint> &GetRallyPointList() {
		return rallypoint_list;
	}
	...
	virtual bool UnderCursorCanAttack(int &map_x, int &map_y) {
		return true;
	}
	virtual bool UnderCursorFortCanEnter(int &map_x, int &map_y) {
		return false;
	}
	...
	int GetInitialHealthPercent() {
		return initial_health_percent;
	}
	...
	void SetGroupExperience(unsigned char exp);
	...
	double SpeedOffsetPercentInv() {
		return 1.0 / SpeedOffsetPercent();
	}
	...
	void ProcessMoveWP(vector<waypoint>::iterator &wp, double time_dif, bool is_new, ZOLists &ols, ZMap &tmap,
			bool stoppable);
	...
	bool ProcessMoveOrKillWP(double time_dif, ZMap &tmap, vector<waypoint>::iterator &wp, ZOLists &ols, bool stoppable =
			true);
	bool ProcessMove(double time_dif, ZMap &tmap, bool stoppable = true) {
		int sx, sy;
		return ProcessMove(time_dif, tmap, sx, sy, stoppable);
	}
	bool ProcessMove(double time_dif, ZMap &tmap, int &stop_x, int &stop_y, bool stoppable = true);
	bool ReachedTarget();
	void SetTarget(int x, int y);
	void SetTarget() {
		SetTarget(cur_wp_info.x, cur_wp_info.y);
	}
	...
	virtual void CreateBuildingQueueData(char *&data, int &size) {
		data = NULL;
		size = 0;
	}
	...
	void SetJustDisembarked(bool just_left_cannon_) {
		just_disembarked = just_left_cannon_;
	}
	virtual bool NearestAttackLoc(int sx, int sy, int &ex, int &ey, int aa_radius, bool is_robot, ZMap &tmap);
	virtual bool IsDestroyableImpass() {
		return false;
	}
	virtual bool CausesImpassAtCoord(int x, int y) {
		return false;
	}
	double DamagedSpeed();
	double RunSpeed();
	bool CanReachTargetRunning(int x, int y);
	virtual bool ShowDamaged() {
		return false;
	}
	virtual bool ShowPartiallyDamaged() {
		return false;
	}
	void AttemptStartRun(int x, int y) {
		if (CanReachTargetRunning(x, y))
			AttemptStartRun();
	}
	void AttemptStartRun();

	//AI stuff
	vector<ZObject*> &GetAIList() {
		return ai_list;
	}
	vector<ZObject*> &GetAIOrigList() {
		return ai_orig_list;
	}
	void AddToAIList(ZObject *obj) {
		ai_list.push_back(obj);
		obj->GetAIList().push_back(this);
	}
private:
...
protected:
	...
	double stamina;
	bool is_running;
	bool display_health_bar;
	unsigned char experience;

	//attacking stuff
	ZObject *m_attacked_object;
