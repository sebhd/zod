...
int ServerSocket::SendMessage(int player, int pack_id, const char *data, int size) {
	if (client_socket[player])
		return client_socket[player]->SendMessage(pack_id, data, size);
	else
		return 0;
}
...
int ServerSocket::SendMessageAscii(int player, int pack_id, const char *data) {
	if (client_socket[player])
		return client_socket[player]->SendMessageAscii(pack_id, data);
	else
		return 0;
}
...
int ServerSocket::CheckConnects() {
	...
	if (temp_id > -1) //got one so lets put him in
		...
		ehandler->ProcessEvent(OTHER_EVENT, CONNECT_EVENT, (char*) connect_ip.c_str(), connect_ip.size() + 1,
