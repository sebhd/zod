...
bool HubButton::WithinCords(int x_, int y_) {
	...
	if (y_ < ty)
		return false;
	if (x_ > tx + button_img[state].GetBaseSurface()->w)
		return false;
	if (y_ > ty + button_img[state].GetBaseSurface()->h)
		return false;
...
void ZHud::GiveSelectedCommand(bool no_way) {
	...
	if (selected_object)
		selected_object->PlayAcknowledgeAnim(portrait, no_way);
}
...
void ZHud::SetSelectedObject(ZObject *selection) {
	...
	if (selected_object) {
		...
		case VEHICLE_OBJECT:
			...
			manned = dynamic_cast<ZMannedObject*>(selected_object);

			if (manned) {
				render_unit_label = &robot_unit_label[manned->GetDriverType()][team];
			}
...
bool ZHud::OverPortrait(int rx, int ry) {
	...
	if (rx < port_x)
		return false;
	if (rx > port_x + ZPORTRAIT_BASE_WIDTH
		) return false;
	if (ry < port_y)
		return false;
	if (ry > port_y + ZPORTRAIT_BASE_HEIGHT
		) return false;
...
void ZHud::DoRender(SDL_Surface *dest, int screen_w, int screen_h) {
	...
	if (rerender_main) {
		//render bottom
		{
			...
			//center
			if (main_hud_bottom_left.GetBaseSurface() && main_hud_bottom_center.GetBaseSurface()
					&& main_hud_bottom_right.GetBaseSurface()) {
				int start_x, end_x;
...
void ZHud::RenderUnitAmountBar(SDL_Surface *dest, int off_x, int off_y) {
	...
	if (!rerender_unit_amount)
		return;

	if (!unit_amount_bar[team].GetBaseSurface())
		return;
