...
void GWPUnitSelector::SetDrawObject() {
	...
	if (is_only_selector || build_state == BUILDING_SELECT) {
		...
		// TODO 4: Shortcut pointer to build list

		//make sure our select_i is kosher
		if (!buildlist->GetBuildList(building_type, b_level).size()) {
			return;
		}
...
void GWPUnitSelector::ResetDrawObjectTo(unsigned char ot, unsigned char oid) {
	...
	case VEHICLE_OBJECT:
		...
		//for fun
		if (draw_obj)
			draw_obj->SetDirection(rand() % MAX_ANGLE_TYPES);
		break;
	case ROBOT_OBJECT:
		switch (oid) {
		case GRUNT:
...
void GWPUnitSelector::DoRender(ZMap &the_map, SDL_Surface *dest, int shift_x, int shift_y) {
	...
	if (draw_obj) {
		...
		if (draw_obj->CreateHoverNameImages().GetBaseSurface()) {
			the_map.RenderZSurface(&draw_obj->CreateHoverNameImages(), tx + (28 - 3) - (draw_obj->CreateHoverNameImages().GetBaseSurface()->w >> 1), ty + (61 - 19));
		}
	}
}
...
void GWPUnitSelector::DrawPercentageBar(ZMap &the_map, SDL_Surface *dest, int tx, int ty) {
	...
	if (!building_obj || (build_state == BUILDING_SELECT) || is_only_selector) {
		return;
	}
...
void GWPUnitSelector::Process() {
	...
	if (building_obj) {
		...
		if (building_obj->GetBuiltCannonList().size()) {
			build_state = BUILDING_PLACE;
		}

		if (building_obj->IsDestroyed()) {
			build_state = BUILDING_PAUSED;
		}
	...
	// TODO 5: Perhaps don't do this here each time?
	SetDrawObject();
...
void GWPUnitSelector::SetSelection(unsigned char ot, unsigned char oid) {
	...
	vector<buildlist_object> &blist = buildlist->GetBuildList(building_type, building_obj->GetLevel());

	// Search the build list for the object that we want to select and apply only if
	// it was found:
...
bool GWPUnitSelector::UnClick(int x_, int y_) {
	...
	if (up_button.UnClick(local_x, local_y)) {
		DoUpButton();
	}

	// Handle down button click event:
	if (down_button.UnClick(local_x, local_y)) {
		DoDownButton();
	}
...
/*
	if (is_only_selector || build_state == BUILDING_SELECT) {
		load_fus = IsWithinPortrait(local_x, local_y);
	}
*/
	load_fus = IsWithinPortrait(local_x, local_y);
...
bool GWPUnitSelector::IsWithinPortrait(int x, int y) {
	if (x < 2)
		return false;
	if (y < 2)
		return false;
...
void GWPUnitSelector::SetSelectedIndex(int i) {

	if (!building_obj || !buildlist) {
		return;
	}

	unsigned int size = buildlist->GetBuildList(building_type, building_obj->GetLevel()).size();
