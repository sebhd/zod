...
CGatling::CGatling(ZTime *ztime_, ZSettings *zsettings_, bool just_placed) :
		ZCannon(ztime_, zsettings_) {
	hover_name = "Gatling";
	object_name = "gatling";

	can_snipe = true;

	InitTypeId(m_object_type, GATLING);
...
void CGatling::Init() {
	...
	for (i = 1; i < MAX_TEAM_TYPES; i++)
		for (j = 0; j < MAX_ANGLE_TYPES; j++) {
			sprintf(filename_c, "assets/units/cannons/gatling/fire_%s_r%03d_n00.png", team_type_string[i].c_str(),
					ROTATION[j]);
			...
			sprintf(filename_c, "assets/units/cannons/gatling/fire_%s_r%03d_n01.png", team_type_string[i].c_str(),
					ROTATION[j]);
...
int CGatling::Process() {
	...
	switch (mode) {
	case JUST_PLACED_MODE:
		if (the_time - last_process_time < 0.1)
			break;
	...
	case ROTATING_MODE:
		...
		if (m_attacked_object) {
			...
			if (renderfire) {
				...
				if (effect_list)
					effect_list->push_back((ZEffect*) (new EBullet(ztime, owner, lx, ly, tx, ty)));
		...
		if (the_time - last_process_time < 1.0)
			break;
		if (owner == NULL_TEAM)
			break;
		last_process_time = the_time;
...
void CGatling::FireTurrentMissile(int x_, int y_, double offset_time) {
	if (effect_list)
		effect_list->insert(effect_list->begin(),
				(ZEffect*) (new ECannonDeath(ztime, loc.x, loc.y, ECANNONDEATH_GATLING, x_, y_, offset_time)));
