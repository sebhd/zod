...
BRepair::BRepair(ZTime *ztime_, ZSettings *zsettings_, planet_type palette_) :
		ZBuilding(ztime_, zsettings_, palette_) {
	width = 5;
	height = 4;
	width_pix = width * 16;
...
void BRepair::DoDeathEffect(bool do_fire_death, bool do_missile_death) {
	...
	//fireballs
	{
		...
		for (i = 0; i < fireballs; i++) {
			...
			if (effect_list)
				effect_list->push_back((ZEffect*) (new ESideExplosion(ztime, sx, sy, 1.3)));
		}
	}
		...
		case 0:
			if (effect_list)
				effect_list->push_back(
						(ZEffect*) (new ETurrentMissile(ztime, sx, sy, ex, ey, offset_time,
								ETURRENTMISSILE_BUILDING_PEICE0)));
			break;
		case 1:
			if (effect_list)
				effect_list->push_back(
						(ZEffect*) (new ETurrentMissile(ztime, sx, sy, ex, ey, offset_time,
								ETURRENTMISSILE_BUILDING_PEICE1)));
			break;
		}
	}
...
bool BRepair::SetRepairUnit(ZObject *unit_obj) {
	...
	if (repairing_unit)
		return false;
	if (!CanRepairUnit(unit_obj->GetOwner()))
		return false;
	...
	ZMannedObject* manned = dynamic_cast<ZMannedObject*>(unit_obj);

	if (manned) {

		driver_type = manned->GetDriverType();
		driver_info = manned->GetDrivers();
	}
...
bool BRepair::RepairUnit(double &the_time, unsigned char &ot, unsigned char &oid, int &driver_type_,
		vector<driver_info_s> &driver_info_, vector<waypoint> &rwaypoint_list_) {
	if (!repairing_unit)
		return false;
	if (the_time < repair_time)
		return false;
