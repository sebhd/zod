...
// TODO 6: Reimplement the entire settings loading code (use xml?)
...
void ZSettings::SetDefaults() {
	...
	robot_settings[oid].max_run_time = run_past_radius * robot_settings[oid].attack_radius
			/ robot_settings[oid].move_speed;
	...
	robot_settings[oid].max_run_time = 0.5 * run_past_radius * robot_settings[oid].attack_radius
			/ robot_settings[oid].move_speed;
	...
	vehicle_settings[oid].max_run_time = run_past_radius * vehicle_settings[oid].attack_radius
			/ vehicle_settings[oid].move_speed;
	...
	vehicle_settings[oid].max_run_time = 0.7 * run_past_radius * vehicle_settings[oid].attack_radius
			/ vehicle_settings[oid].move_speed;
	...
	vehicle_settings[oid].max_run_time = 0.5 * run_past_radius * vehicle_settings[oid].attack_radius
			/ vehicle_settings[oid].move_speed;
...
void ZUnit_Settings::CensorNegatives() {
	...
	if (attack_damage < 0)
		attack_damage = 0;
	if (attack_damage_chance < 0)
		attack_damage_chance = 0;
	if (attack_damage_radius < 0)
		attack_damage_radius = 0;
	if (attack_missile_speed < 0)
		attack_missile_speed = 0;
	if (attack_speed < 0)
		attack_speed = 0;
	if (attack_snipe_chance < 0)
	...
	if (attack_damage_chance > 1.0)
		attack_damage_chance = 1.0;
	if (attack_snipe_chance > 1.0)
		attack_snipe_chance = 1.0;
}
...
void ZSettings::CensorSettings() {
	...
	for (int i = 0; i < MAX_ROBOT_TYPES; i++)
		robot_settings[i].CensorNegatives();
	for (int i = 0; i < MAX_VEHICLE_TYPES; i++)
		vehicle_settings[i].CensorNegatives();
	for (int i = 0; i < MAX_CANNON_TYPES; i++)
		cannon_settings[i].CensorNegatives();
	if (agro_distance < 0)
		agro_distance = 0;
	if (auto_grab_vehicle_distance < 0)
		auto_grab_vehicle_distance = 0;
	if (auto_grab_flag_distance < 0)
		auto_grab_flag_distance = 0;
	if (building_auto_repair_time < 0)
		building_auto_repair_time = 0;
	if (building_auto_repair_random_additional_time < 0)
		building_auto_repair_random_additional_time = 0;
	if (max_turrent_horizontal_distance < 0)
		max_turrent_horizontal_distance = 0;
	if (max_turrent_vertical_distance < 0)
		max_turrent_vertical_distance = 0;
	if (grenades_per_box < 0)
		grenades_per_box = 0;
	if (grenades_per_box > 99)
		grenades_per_box = 99;

	if (grenade_damage < 0)
		grenade_damage = 0;
	if (grenade_damage_radius < 0)
		grenade_damage_radius = 0;
	if (map_item_turrent_damage < 0)
		map_item_turrent_damage = 0;
	if (partially_damaged_unit_speed < 0)
		partially_damaged_unit_speed = 0;
	if (damaged_unit_speed < 0)
		damaged_unit_speed = 0;
	if (run_unit_speed < 0)
		run_unit_speed = 0;
	if (run_recharge_rate < 0)
		run_recharge_rate = 0;
	if (run_recharge_rate > 1)
		run_recharge_rate = 1;
	if (hut_animal_max < 0)
		hut_animal_max = 0;
	if (hut_animal_min < 0)
		hut_animal_min = 0;
	if (hut_animal_roam_distance < 0)
		hut_animal_roam_distance = 0;
}
...
void ZUnit_Settings::SaveLine(FILE *fp, string obj_name) {
	fprintf(fp, "\n");
	fprintf(fp, "%s.can_be_sniped=%d\n", obj_name.c_str(), can_be_sniped);
...
void ZUnit_Settings::ReadEntry(string variable, string value) {
	...
	if (variable == "can_be_sniped")
		can_be_sniped = atoi(value.c_str());
	else if (variable == "smartness") {
		smartness = atoi(value.c_str());
	}
	else if (variable == "group_amount")
...
ZUnit_Settings &ZSettings::GetUnitSettings(unsigned char ot, unsigned char oid) {
	...
	switch (ot) {
	case CANNON_OBJECT:
		if (oid >= MAX_CANNON_TYPES)
			break;
		return cannon_settings[oid];
		break;
	case VEHICLE_OBJECT:
		if (oid >= MAX_VEHICLE_TYPES)
			break;
