...
void ZGuiScrollBar::Init() {
	...
	if (!main_top_img.GetBaseSurface())
		return;
	if (!main_center_img.GetBaseSurface())
		return;
	if (!main_bottom_img.GetBaseSurface())
		return;
	if (!inner_top_img.GetBaseSurface())
		return;
	if (!inner_center_img.GetBaseSurface())
		return;
	if (!inner_bottom_img.GetBaseSurface())
		return;
...
void ZGuiScrollBar::SetPercentViewable(double percent_viewable_) {
	...
	if (percent_viewable > 1.0)
		percent_viewable = 1.0;
	if (percent_viewable < 0)
		percent_viewable = 0;
}
...
void ZGuiScrollBar::RenderStrip(ZMap &the_map, SDL_Surface *dest, int x, int y, int h_, ZSDL_Surface &top_img,
		ZSDL_Surface &center_img, ZSDL_Surface &bottom_img) {
	int ty, by;
...
void ZGuiTextBox::KeyPress(int c) {
	if (c == 8) //delete key
		...
		if (max_text != -1 && text.length() >= max_text)
			return;
		if (good_chars_only && !good_user_char(c))
			return;
...
bool ZGuiButton::Click(int x, int y) {
	...
	if (!button_img[type][state].GetBaseSurface())
		return false;
	...
	if (y < y_offset)
		return false;
	if (x > x_offset + button_img[type][state].GetBaseSurface()->w)
		return false;
	if (y > y_offset + button_img[type][state].GetBaseSurface()->h)
		return false;
