...
void ZPortrait_Unit_Graphics::Load(robot_type oid, team_type team, ZPortrait_Unit_Graphics &bc) {
	...
	//load em
	i = 0;
	sprintf(filename_c, "assets/other/hud/portraits/%s_%s/SHEADBI%d_%04d.png", robot_type_string[oid].c_str(),
			team_type_string[team].c_str(), old_z_face_id, i++);
...
void ZPortrait::SetObject(ZObject *obj) {
	...
	case VEHICLE_OBJECT:
	case CANNON_OBJECT:
		manned = dynamic_cast<ZMannedObject*>(obj);

		if (manned) {
			SetRobotID(manned->GetDriverType());
			SetInVehicle(true);
		}
		break;
	}
...
void ZPortrait::StartAnim(int anim_) {
	if (!anim_info[anim_].frame_list.size())
		return;
...
void ZPortrait::StartRandomAnim() {
	...
	switch (rand() % 13) {
	case 0:
		StartAnim(BLINK_ANIM);
		break;
	case 1:
		StartAnim(WINK_ANIM);
		break;
	case 2:
		StartAnim(SURPRISE_ANIM);
		break;
	case 3:
		StartAnim(ANGER_ANIM);
		break;
	case 4:
		StartAnim(GRIN_ANIM);
		break;
	case 5:
		StartAnim(SCARED_ANIM);
		break;
	case 6:
		StartAnim(EYES_LEFT_ANIM);
		break;
	case 7:
		StartAnim(EYES_RIGHT_ANIM);
		break;
	case 8:
		StartAnim(EYES_UP_ANIM);
		break;
	case 9:
		StartAnim(EYES_DOWN_ANIM);
		break;
	case 10:
		StartAnim(WHISTLE_ANIM);
		break;
	case 11:
		StartAnim(LOOK_LEFT_ANIM);
		break;
	case 12:
		StartAnim(LOOK_RIGHT_ANIM);
		break;
	}
}
...
int ZPortrait::Process() {
	...
	if (cur_anim != -1) {
		...
		if (the_time >= next_random_anim_time)
			StartRandomAnim();
...
void ZPortrait::PlayAnimSound() {
	...
	case YES_SIR_ANIM:
		...
		break;
	case YES_SIR3_ANIM:
		ZSoundEngine::PlayWav(YES_SIR3_SND);
		break;
	case UNIT_REPORTING1_ANIM:
		ZSoundEngine::PlayWav(UNIT_REPORTING1_SND);
		break;
	case UNIT_REPORTING2_ANIM:
		...
		break;
	case GRUNTS_REPORTING_ANIM:
		ZSoundEngine::PlayWav(GRUNTS_REPORTING_SND);
		break;
	case PSYCHOS_REPORTING_ANIM:
		ZSoundEngine::PlayWav(PSYCHOS_REPORTING_SND);
		break;
	case SNIPERS_REPORTING_ANIM:
		ZSoundEngine::PlayWav(SNIPERS_REPORTING_SND);
		break;
	case TOUGHS_REPORTING_ANIM:
		ZSoundEngine::PlayWav(TOUGHS_REPORTING_SND);
		break;
	case LASERS_REPORTING_ANIM:
		ZSoundEngine::PlayWav(LASERS_REPORTING_SND);
		break;
	case PYROS_REPORTING_ANIM:
		ZSoundEngine::PlayWav(PYROS_REPORTING_SND);
		break;
	case WERE_ON_OUR_WAY_ANIM:
		ZSoundEngine::PlayWav(WERE_ON_OUR_WAY_SND);
		break;
	case HERE_WE_GO_ANIM:
		ZSoundEngine::PlayWav(HERE_WE_GO_SND);
		break;
	case YOUVE_GOT_IT_ANIM:
		ZSoundEngine::PlayWav(YOU_GOT_IT_SND);
		break;
	case MOVING_IN_ANIM:
		ZSoundEngine::PlayWav(MOVING_IN_SND);
		break;
	case OKAY_ANIM:
		ZSoundEngine::PlayWav(OKAY_SND);
		break;
	case ALRIGHT_ANIM:
		ZSoundEngine::PlayWav(ALRIGHT_SND);
		break;
	case NO_PROBLEM_ANIM:
		ZSoundEngine::PlayWav(NO_PROBLEM_SND);
		break;
	case OVER_N_OUT_ANIM:
		ZSoundEngine::PlayWav(OVER_AND_OUT_SND);
		break;
	case AFFIRMATIVE_ANIM:
		ZSoundEngine::PlayWav(AFFIRMATIVE_SND);
		break;
	case GOING_IN_ANIM:
		ZSoundEngine::PlayWav(GOING_IN_SND);
		break;
	case LETS_DO_IT_ANIM:
		ZSoundEngine::PlayWav(LETS_DO_IT_SND);
		break;
	case LETS_GET_EM_ANIM:
		ZSoundEngine::PlayWav(LETS_GET_THEM_SND);
		break;
	case WERE_UNDER_ATTACK_ANIM:
		ZSoundEngine::PlayWav(WERE_UNDER_ATTACK_SND);
		break;
	case I_SAID_WERE_UNDER_ATTACK_ANIM:
		ZSoundEngine::PlayWav(I_SAID_WERE_UNDER_ATTACK_SND);
		break;
	case HELP_HELP_ANIM:
		ZSoundEngine::PlayWav(HELP_HELP_SND);
		break;
	case THEYRE_ALL_OVER_US_ANIM:
		ZSoundEngine::PlayWav(THEYRE_ALL_OVER_US_SND);
		break;
	case WERE_LOSEING_IT_ANIM:
		ZSoundEngine::PlayWav(WERE_LOSING_IT_SND);
		break;
	case AAAHHH_ANIM:
		ZSoundEngine::PlayWav(AAAHHH_SND);
		break;
		//where is the "oh my god" anim? -> OH_MY_GOD_SND
	case FOR_CHRIST_SAKE_ANIM:
		ZSoundEngine::PlayWav(FOR_CHRIST_SAKE_SND);
		break;
	case YOURE_JOKING_ANIM:
		ZSoundEngine::PlayWav(YOUR_JOKING_SND);
		break;
	case TARGET_DESTROYED_ANIM:
		ZSoundEngine::PlayWav(TARGET_DESTROYED_SND);
		break;
	case YES_SIR_SALUTE_ANIM:
		if (rand() % 2)
			...
			ZSoundEngine::PlayWav(YES_SIR2_SND);
		break;
	case GOING_IN_THUMBS_UP_ANIM:
		ZSoundEngine::PlayWav(GOING_IN_SND);
		break;
	case FORGET_IT_ANIM:
		ZSoundEngine::PlayWav(FORGET_IT_SND);
		break;
	case GET_OUTTA_HERE_ANIM:
		ZSoundEngine::PlayWav(GET_OUTTA_HERE_SND);
		break;
	case GOOD_HIT_ANIM:
		ZSoundEngine::PlayWav(GOOD_HIT_SND);
		break;
	case NO_WAY_ANIM:
		ZSoundEngine::PlayWav(NO_WAY_SND);
		break;
	case NICE_ONE_ANIM:
		ZSoundEngine::PlayWav(NICE_ONE_SND);
		break;
	case OH_YEAH_ANIM:
		ZSoundEngine::PlayWav(OH_YEAH_SND);
		break;
	case GOTCHA_ANIM:
		ZSoundEngine::PlayWav(GOTCHA_SND);
		break;
	case SMOKIN_ANIM:
		ZSoundEngine::PlayWav(SMOKIN_SND);
		break;
	case COOL_ANIM:
		ZSoundEngine::PlayWav(COOL_SND);
		break;
	case WIPE_OUT_ANIM:
		ZSoundEngine::PlayWav(WIPE_OUT_SND);
		break;
	case TERRITORY_TAKEN_ANIM:
		ZSoundEngine::PlayWav(TERRITORY_TAKEN_SND);
		break;
	case FIRE_EXTINGUISHED_ANIM:
		ZSoundEngine::PlayWav(FIRE_EXTINGUISHED_SND);
		break;
	case GUN_CAPTURED_ANIM:
		ZSoundEngine::PlayWav(GUN_CAPTURED_SND);
		break;
	case VEHICLE_CAPTURED_ANIM:
		ZSoundEngine::PlayWav(VEHICLE_CAPTURED_SND);
		break;
	case GRENADES_COLLECTED_ANIM:
		ZSoundEngine::PlayWav(GRENADES_COLLECTED_SND);
		break;
	case ENDW1_ANIM:
	case ENDW2_ANIM:
	case ENDW3_ANIM:
		next_end_anim = rand() % 6;
		while (next_end_anim == last_end_anim)
			next_end_anim = rand() % 6;
		last_end_anim = next_end_anim;
		ZSoundEngine::PlayWav(YEEHAA_END_SND + next_end_anim);
		break;
	case ENDL1_ANIM:
	case ENDL2_ANIM:
	case ENDL3_ANIM:
		...
		while (next_end_anim == last_end_anim)
			next_end_anim = rand() % 7;
		last_end_anim = next_end_anim;
		ZSoundEngine::PlayWav(YOURE_CRAP_LOSE_SND + next_end_anim);
		break;
	}
}
